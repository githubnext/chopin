# Hosted Copilot agent

Hosted channels use the GitHub OAuth token of the first editor who invokes the
...
Reset and logout abort and discard any active hosted SDK session. Permission
callbacks also recheck the owner session, ownership generation and repository
write access before each custom or MCP tool call.

The shared Copilot runtime runs in SDK `mode: "empty"`. Each hosted SDK session
...
- bounded file, tree, code-search and commit-history tools fixed to the selected
  repository; and
- read-only GitHub MCP calls whose arguments explicitly match that repository.
...
current plan is read through `read_plan`. No SDK session ID is stored for hosted
channels.

OAuth App access tokens are supported by the Copilot SDK and charge the owning
user's Copilot subscription. Chopin cannot prove entitlement at login: a user
without Copilot sees the provider failure on their first planner turn and
remains owner until the channel is reset.

The runtime is started lazily. `AGENT=off` keeps the hosted planner hidden and
does not start Copilot CLI. Legacy `AUTH_DRIVER=off` planning retains its local
checkout, global token and startup probe.
