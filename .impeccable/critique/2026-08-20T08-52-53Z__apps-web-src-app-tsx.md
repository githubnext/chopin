...
**Evidence:** 39 raw states, ten annotated plates, code inspection, production build, static checks\
**Overall verdict:** **24/40 — acceptable, with four product-critical failures**
...
This is not a redesign problem yet. It is a containment, hierarchy, and interaction-contract problem. Fix the four P1 issues before adding more surface area.

![Inventory contact sheet showing all 39 captured states](./annotated/00-state-inventory.png)

## Design health score

| Nielsen heuristic               |     Score | Assessment                                                                                                                             |
| ------------------------------- | --------: | -------------------------------------------------------------------------------------------------------------------------------------- |
| Visibility of system status     |       2/4 | Connection and planner states exist, but presence counts connections as people and gives a false account of who is here.               |
...
| **Total**                       | **24/40** | **Acceptable — significant improvements required.**                                                                                    |
...
## P1 — One person becomes several people when they open more tabs

![Annotated duplicate same-user presence](./annotated/09-duplicate-same-user-presence.png)

Three tabs authenticated as the same `e2e` account produce three faces in the top-right presence stack. The accessible name is equally literal: `People here: e2e, e2e, e2e`. This is not a cosmetic duplicate. Presence is supposed to answer “who is here?”, and the interface answers with a connection count instead.

The underlying model explains the result. Rooms are keyed by client ID because every socket must remain independently manageable in [`rooms.ts`](../../apps/server/src/rooms.ts#L15). `members()` then returns every client identity, and the header renders each entry with `key={member.client}` in [`app.tsx`](../../apps/web/src/app.tsx#L60).

**Change:** Keep connection membership per client for transport, cleanup, cursors, and awareness. Derive the header's people summary by unique verified GitHub handle. Render one face and one accessible-name entry per person; optionally retain the number of open connections as non-primary metadata for debugging.

**Acceptance test:** Opening one account in three tabs still renders one person. Opening two different accounts renders two people. Closing any subset of one person's tabs does not remove their face until their last connection leaves.
...
## P2 — The document switcher trigger takes over the header

![Annotated oversized document switcher trigger](./annotated/10-document-switcher-width.png)

The document menu itself is a sensible 360px. The closed trigger is the problem: at 1440px it measures **1,191px wide** for the label “Test 33333333”. Focus and hover therefore paint almost the entire remaining header, making a small navigation control look like a full-width field.

Both layout levels opt into growth: the header wrapper uses `flex-1` in [`app.tsx`](../../apps/web/src/app.tsx#L50), and the trigger uses `flex-1` again in [`document-picker.tsx`](../../apps/web/src/document-picker.tsx#L249).

**Change:** Fit the trigger to its label and padding, with a maximum width that truncates genuinely long document names. Keep the 360px menu width and its current anchoring. Free header space should remain empty rather than becoming part of the switcher's hit area.

**Acceptance test:** Short labels produce short triggers; long labels truncate at the maximum; opening the menu does not change the trigger width; repository, document, and presence controls never overlap at compact desktop widths.
...
| Presence/navigation     | [38 duplicate same-user presence](./images/38-duplicate-same-user-presence.png), [39 full-width document trigger](./images/39-document-switcher-full-width.png)                                                                                                                       |

Screenshots 21 and 24 intentionally show the compact comment sheet. Screenshot 33 is the clean compact plan. Screenshots 38 and 39 use three same-account tabs and measured header geometry. Repeated question counts and duplicate comment buttons in earlier captures were caused by development fixtures reinjecting on reopen; they are excluded from the product findings.
...
4. Deduplicate the header's presence summary by person without merging the underlying client connections.

### Next — clarify the product

5. Build an operational conversation empty state and explain `@ai` versus room messages.
6. Rebalance desktop workspace width and consolidate the Plan/Decisions hierarchy.
7. Fit the document switcher trigger to its content while preserving truncation.
8. Add a real modal depth transition and stronger action language to comment sheets.
9. Improve error recovery with context, retry, and channel-list navigation.

### Then — improve craft and speed

10. Teach slash insertion and reveal table tools from selection, not hover hunting.
11. Add route-level bundle splitting and performance budgets.
12. Refine login, repository, and channel surfaces with more useful context and denser desktop composition.
13. Pair entrance and exit motion where spatial continuity matters; remove it from context menus.
